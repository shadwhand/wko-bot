(function () {
  ...
  WKO5Registry.registerFactory('if-floor', function (container, api) {
    function render(data) {
      ...
      var color = ifFlag.severity === 'RED' || ifFlag.severity === 'red' ? '#f85149' : '#d29922';
      container.innerHTML = '<div style="padding:16px;border-left:4px solid ' + color + ';border-radius:6px;background:var(--bg-secondary);">' +
        ...
        '<div style="color:var(--text-secondary);font-size:0.85rem;margin-top:6px;">' + escapeHtml(ifFlag.message || ifFlag.description || 'Endurance rides too intense') + '</div>' +
        (ifFlag.value != null ? '<div class="mono" style="margin-top:6px;font-size:0.85rem;">IF: ' + Number(ifFlag.value).toFixed(2) + '</div>' : '') +
      '</div>';
    }
