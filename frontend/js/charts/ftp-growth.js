  ...
  WKO5Registry.registerFactory('ftp-growth', function (container, api) {
    ...
    function render(data) {
      ...
      var cards = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:8px;margin-bottom:12px;font-size:0.8rem;">' +
        '<div style="text-align:center;"><div class="mono" style="font-size:1.4rem;font-weight:600;color:var(--accent);">' + fmtNum(data.current_ftp) + '</div><div style="color:var(--text-secondary);">Current FTP</div></div>' +
        '<div style="text-align:center;"><div class="mono" style="font-size:1.4rem;font-weight:600;">' + fmtNum(data.growth_rate, 1) + '</div><div style="color:var(--text-secondary);">W/year</div></div>' +
        ...
        (data.tte != null ? '<div style="text-align:center;"><div class="mono" style="font-size:1.4rem;font-weight:600;">' + fmtNum(data.tte, 1) + '</div><div style="color:var(--text-secondary);">TTE (min)</div></div>' : '') +
      '</div>';
      ...
      var ftps = history.map(function (h) { return h.ftp; });
      ...
      g.selectAll('circle').data(history).enter().append('circle')
        ...
        .attr('cy', function (d) { return y(d.ftp); })
        .attr('r', 4)
        .attr('fill', 'var(--accent)')
        .attr('opacity', 0.7);

      // Log fit curve
      if (data.log_fit && data.log_fit.a != null && data.log_fit.b != null) {
        var a = data.log_fit.a, b = data.log_fit.b;
        ...
        for (var i = 0; i <= 50; i++) {
          ...
          var predicted = a * Math.log(weeks + 1) + b;
          curvePoints.push({ date: new Date(t), ftp: predicted });
        }
        var line = d3.line()
          .x(function (d) { return x(d.date); })
          .y(function (d) { return y(d.ftp); })
