# Route Storage + Similarity Matching — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Persist GPX routes with segments and ride plans, match historical activities to stored routes using discrete Frechet distance on downsampled GPS tracks, and link ride plans/results to routes for longitudinal analysis.

**Architecture:** One new module `routes.py` handles route storage, GPS downsampling, Frechet distance computation, and activity-route linking. DB tables store routes, downsampled points, segments, ride plans, and plan results. Activity GPS coordinates are converted from Garmin semicircles to decimal degrees for matching.

**Tech Stack:** Python 3, numpy, SQLite, defusedxml

**Existing:** `wko5/` with 157 tests. `segments.py` already has `analyze_gpx()`. DB has 1,431 activities with GPS data. Garmin stores lat/lon as semicircles (multiply by 180/2^31 for degrees).
...
```
wko5/
  routes.py    # Route storage, GPS downsampling, Frechet distance, activity linking
```

Single module — route storage and matching are tightly coupled (matching needs the stored points).

---

## Task 1: Route storage and GPX import

**Files:**
- Create: `wko5/routes.py`
...
Run: `source /tmp/fitenv/bin/activate && pytest tests/test_routes.py -v`
...
```bash
git add wko5/routes.py tests/test_routes.py
git commit -m "feat: add route storage, GPS Frechet matching, activity linking, ride plan persistence"
```

---

## Task 2: API endpoints + exports
...
```python
from wko5.routes import save_route, get_route, get_all_routes, get_route_history, get_ride_plans
...
```bash
git add wko5/api/routes.py wko5/__init__.py tests/test_api.py docs/superpowers/plans/2026-03-21-route-matching.md
git commit -m "feat: add route API endpoints and exports"
```
